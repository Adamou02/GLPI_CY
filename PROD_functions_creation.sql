...
IS v_count NUMBER;
BEGIN
    EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM GLPI_PROD.' || p_table_name || ' WHERE '|| p_column_name ||' = :1' 
...
EXCEPTION
     WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('La table GLPI_PROD.' || p_table_name || ' n''existe pas.');
        RETURN FALSE;
    WHEN OTHERS THEN
        IF SQLCODE = -904 THEN
...
CREATE OR REPLACE FUNCTION GLPI_PROD.GET_ID_WHERE(
    p_value IN VARCHAR2, 
