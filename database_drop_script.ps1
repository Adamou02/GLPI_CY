...
$DropFileOrder = @(
    ...
    "PROD\PROD_drop_users.sql",
    "TEST\TEST_drop_users.sql",
    "PROD\PROD_drop_roles.sql",
    "TEST\TEST_drop_roles.sql",
...
foreach ($DropSqlFileName in $DropFileOrder) {
    ...
    $Prefix = $DropSqlFileName.Substring(0, 4)
    $Credential = $CredentialMapping[$Prefix]
    $LogFile = "$LogFileDirectory\$($DropSqlFileName.SubString(5).Replace(".sql",".log"))"
