...
CREATE TABLE GLPI_PROD.USERS (
    ...
    company VARCHAR2(50),
    fk_location INT,
    FOREIGN KEY (fk_role) REFERENCES GLPI_PROD.REF_role(role_id),
...
CREATE TABLE GLPI_PROD.TICKETS (
    ...
    FOREIGN KEY (fk_location) REFERENCES GLPI_PROD.LOCATIONS(location_id),
    FOREIGN KEY (fk_assigned_group) REFERENCES GLPI_PROD.REF_group(group_id),
    FOREIGN KEY (fk_status) REFERENCES GLPI_PROD.REF_status(status_id),
...
-- les <fk_value> sont des INT et correspondent � la valeur de l�id de la table correspondante. Ce sont des cl�s �trang�res li�es � d'autres tables, assurant l'int�grit� r�f�rentielle entre les tables.COMMIT;
COMMIT;
exit;